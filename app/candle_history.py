...
async def fetch_bybit_candles(symbol: str, interval: str, limit: int = 500) -> Tuple[List[dict], str]:
    params = {
        ...
        "interval": BYBIT_INTERVALS.get(str(interval), "1"),
        "limit": str(limit),
    }
    async with httpx.AsyncClient(timeout=15.0, headers={"User-Agent": "WhaleX/2.0"}) as client:
...
async def fetch_okx_candles(symbol: str, interval: str, limit: int = 500) -> Tuple[List[dict], str]:
    ...
    params = {
        "instId": inst_id,
        "bar": OKX_INTERVALS.get(str(interval), "1m"),
