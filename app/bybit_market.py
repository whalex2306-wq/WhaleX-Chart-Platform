...
class BybitMarketStream:
    def __init__(self, symbol='BTCUSDT', interval='1', bucket=100.0, min_m=5.0, max_lines=10, depth=200, publish_ms=750):
        self.symbol = symbol.upper().strip(); self.interval = INTERVAL_MAP.get(str(interval), '1')
        self.bucket = max(float(bucket), 0.01); self.min_m = max(float(min_m), 0.0)
        self.max_lines = max(1, min(int(max_lines), 50)); self.depth = depth if depth in (50,200,500) else 200
    ...
    def apply_orderbook(self, msg: dict) -> None:
        ...
        for price_s, qty_s in data.get('b', []) or []:
            ...
            if qty <= 0: self.book['bid'].pop(price, None)
            else: self.book['bid'][price] = qty
        for price_s, qty_s in data.get('a', []) or []:
            price = float(price_s); qty = float(qty_s)
            if qty <= 0: self.book['ask'].pop(price, None)
            else: self.book['ask'][price] = qty
    ...
    def update_candle(self, msg: dict) -> Optional[dict]:
        ...
        k = arr[0]
        try:
            candle = {'time': int(int(k['start'])/1000), 'open': float(k['open']), 'high': float(k['high']), 'low': float(k['low']), 'close': float(k['close']), 'volume': float(k.get('volume', 0)), 'confirm': bool(k.get('confirm', False))}
        except Exception: return None
        self.latest_candle = candle; return candle
