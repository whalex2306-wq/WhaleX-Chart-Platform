...
class LiquidityLine:
    ...
    def behavior(self) -> str:
        ...
        if self.previous_liquidity_m <= 0: return 'Holding'
        change_pct = ((self.liquidity_m - self.previous_liquidity_m) / self.previous_liquidity_m) * 100.0
        if change_pct >= 15: return 'Building'
        if change_pct <= -15: return 'Fading'
        if age >= 180: return 'Persistent'
        return 'Holding'
...
class LiquidityMemory:
    ...
    def update(self, side: str, raw_lines: List[Tuple[float, float, int]], min_m: float, max_lines: int) -> List[LiquidityLine]:
        now = time.time(); output: List[LiquidityLine] = []
        for price, qty, levels in raw_lines:
            liquidity_m = (price * qty) / 1_000_000.0
            if liquidity_m < min_m: continue
            key = (side, float(price))
            if key in self.lines:
                old = self.lines[key]
                old.previous_liquidity_m = old.liquidity_m
                old.qty = qty; old.liquidity_m = liquidity_m; old.levels = levels; old.last_seen = now
                line = old
            else:
                line = LiquidityLine(side, float(price), float(qty), float(liquidity_m), int(levels), now, now, 0.0)
                self.lines[key] = line
            output.append(line)
        stale_cutoff = now - 20
        for key in list(self.lines.keys()):
            if self.lines[key].last_seen < stale_cutoff: del self.lines[key]
...
def bucket_orderbook(book: Dict[str, Dict[float, float]], bucket: float):
    ...
    buckets = {'bid': {}, 'ask': {}}; counts = {'bid': {}, 'ask': {}}
    for side in ('bid', 'ask'):
        for price, qty in book.get(side, {}).items():
            if qty <= 0: continue
            b = round((price // bucket) * bucket, 8)
            buckets[side][b] = buckets[side].get(b, 0.0) + qty
            counts[side][b] = counts[side].get(b, 0) + 1
    bid_lines = [(p, q, counts['bid'].get(p, 0)) for p, q in buckets['bid'].items()]
    ask_lines = [(p, q, counts['ask'].get(p, 0)) for p, q in buckets['ask'].items()]
...
def line_to_dict(line: LiquidityLine) -> dict:
    return {'side': line.side, 'price': line.price, 'qty': line.qty, 'liquidity_m': line.liquidity_m, 'levels': line.levels, 'age_seconds': line.age_seconds, 'behavior': line.behavior}
